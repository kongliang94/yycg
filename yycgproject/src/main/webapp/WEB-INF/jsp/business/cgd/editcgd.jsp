...
//采购单另存为
function yycgdtosave(){
	_alert("系统维护中");
}

/* function yycgdtosave_callback(data) {
	...
	_confirm('您确定要删除选择的采购药品吗?',null,
			  function(){
				...
				if(rows.length>0){
					...
					$("#yycgdmxForm").attr("action",'${baseurl}cgd/yycgdmxdelsubmit.action?yycgdid=${yycgd.id}');
...
},
{
	...
	formatter: function(value,row,index){
		if(value){
			return '<input type="hidden" name="yycgdmxCustoms['+index+'].usergysid" value="'+row.usergysid+'" />';
		}else{
			return value;
		}
	}
						...
						<TBODY>
							...
							<tr>
							  <td colspan=4 align=center class=category>
								...
								<a  href="#" onclick="yycgdtosave()" class="easyui-linkbutton" iconCls='icon-save'>采购单另存为</a>
							  </td>
							</tr>
						</TBODY>
					</TABLE>
